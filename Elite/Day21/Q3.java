...
public class Q3{
    ...
    public static BST insert(BST curr, int val){
        ...
        else if(val < curr.data) curr.left = insert(curr.left,val);
        else curr.right = insert(curr.right,val);
        return curr;
    }
    public static BST delete(BST curr, int low, int high){
        if(curr == null) return null;
        if(curr.data < low) return delete(curr.right,low,high);
        if(curr.data > high) return delete(curr.left,low,high);
