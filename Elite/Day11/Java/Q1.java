...
// // X comparments and the smaller cargo ship into Y compartments, where X > Y.
...
import java.util.*;
public class Q1{
    static void backtrack(int[] lc, int[] sc, int[] num, int ind, List<Integer> temp){
        if(temp.size() == sc.length){
            num[0]++;
            return;
        }
        for(int i = ind; i < lc.length; i++){
            if(temp.size() == 0 || temp.get(temp.size()-1) <= (lc[i] + sc[temp.size()])){
                temp.add(lc[i] + sc[temp.size()]);
                backtrack(lc, sc, num, i+1, temp);
    ...
    public static void main(String[] args){
        ...
        int[] lc = new int[l];
        int[] scc = new int[s];
        for(int i = 0; i < l; i++) lc[i] = sc.nextInt();
        for(int i = 0; i < s; i++) scc[i] = sc.nextInt();
        List<Integer> temp = new ArrayList<>();
        int[] num = new int[1];
        backtrack(lc, scc, num, 0, temp);
